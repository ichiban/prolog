...
func New(in io.Reader, out io.Writer) *Interpreter {
	...
	m.Register2("module", engine.Module)
	if err := i.Load(context.Background(), "libraries/prolog.pl"); err != nil {
		panic(err)
	}
	...
	m.Register2("$equal", engine.Equal)
	m.Register2("$not_equal", engine.NotEqual)
	m.Register2("$less_than", engine.LessThan)
	m.Register2("$less_than_or_equal", engine.LessThanOrEqual)
	m.Register2("$greater_than", engine.GreaterThan)
	m.Register2("$greater_than_or_equal", engine.GreaterThanOrEqual)
	...
	m.Register1("$meta_predicate", engine.MetaPredicate)

	_ = i.QuerySolution(`meta_predicate([
  clause(0, ?),
  asserta(0),
  assertz(0),
  retract(0),
