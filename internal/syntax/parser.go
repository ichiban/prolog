...
func MakeVariable(makeVariable func() (term.Cell, error)) ParseOption {
	return func(o *ParseOptions) {
		o.makeVariable = makeVariable
	}
}

func Parse(r io.RuneReader, opts ...ParseOption) iter.Seq2[term.Cell, error] {
	p := newParser(r, opts...)
...
func ParseTerm(r io.RuneReader, opts ...ParseOption) (term.Cell, error) {
	p := newParser(r, opts...)
	t, err := p.Term()
	if err != nil {
		return term.Cell{}, fmt.Errorf("term(): %w", err)
	}
	return t, nil
}

func ParseNumber(r io.RuneReader, opts ...ParseOption) (term.Cell, error) {
	p := newParser(r, opts...)
	return p.Number()
}

func ParseVariable(r io.RuneReader, opts ...ParseOption) (term.Cell, error) {
	p := newParser(r, opts...)
	return p.Variable()
}
...
func (p *parser) Term() (_ term.Cell, err error) {
	t, ok, err := p.term(1201)
	if err != nil {
		return term.Cell{}, fmt.Errorf("term(1201): %w", err)
	}
	if !ok {
		return term.Cell{}, fmt.Errorf("term(1201): %w", &UnexpectedTokenError{token: p.current()})
	}
	...
	case errors.Is(err, io.EOF):
		return term.Cell{}, fmt.Errorf("next(): %w", ErrUnexpectedEOF)
	default:
		p.backup()
		return term.Cell{}, fmt.Errorf("next(): %w", &UnexpectedTokenError{token: p.current()})
	}
...
func (p *parser) Number() (_ term.Cell, err error) {
	var n term.Cell
	t, err := p.next()
	if err != nil {
	...
	default:
		...
		if !ok {
			return term.Cell{}, ErrNotANumber
		}
...
func (p *parser) Variable() (_ term.Cell, err error) {
	var v term.Cell
	t, err := p.next()
	if err != nil {
		return term.Cell{}, err
	...
	case io.EOF:
		return v, nil
	default:
		return term.Cell{}, errors.New("unexpected rune")
	}
}
...
func (p *parser) term(maxPriority int16) (term.Cell, bool, error) {
	var lhs term.Cell
	switch op, ok, err := p.prefix(maxPriority); {
	case err != nil:
		return term.Cell{}, false, fmt.Errorf("prefix(%d): %w", maxPriority, err)
	case !ok:
		lhs, ok, err = p.term0(maxPriority)
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("term0(%d): %w", maxPriority, err)
		}
		if !ok {
			return term.Cell{}, false, nil
		}
	default:
		_, rbp := op.bindingPriorities()
		t, ok, err := p.term(rbp)
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("term(%d): %w", rbp, err)
		}
		if !ok {
			p.backup()
		...
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("PutCompound(%s, %s): %w", op.Name, &Formatter{Arena: p.arena, Term: t}, err)
		}
	}
	...
	for {
		...
		if err != nil {
			...
			return term.Cell{}, false, fmt.Errorf("infix(%d): %w", maxPriority, err)
		}
		if !ok {
			break
		}
...
func (p *parser) term0(maxPriority int16) (term.Cell, bool, error) {
	t, err := p.next()
	if err != nil {
		return term.Cell{}, false, fmt.Errorf("next(): %w", err)
	}
	switch t.kind {
	case tokenOpen, tokenOpenCT:
		t, ok, err := p.openClose()
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("openClose(): %w", err)
		}
		return t, ok, nil
	case tokenInteger:
		i, err := p.integer(1, t.val)
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("integer(1, %s): %w", t.val, err)
		}
		return i, true, nil
	case tokenFloatNumber:
		f, err := p.float(1, t.val)
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("float(1, %s): %w", t.val, err)
		}
		return f, true, nil
	case tokenVariable:
		v, err := p.variable(t.val)
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("variable(%s): %w", t.val, err)
		}
		return v, true, nil
	case tokenOpenList:
		...
		t, ok, err := p.list()
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("list(): %w", err)
		}
		return t, ok, nil
	case tokenOpenCurly:
		...
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("curlyBracketedTerm(): %w", err)
		}
		return t, ok, nil
	...
	if err != nil {
		return term.Cell{}, false, fmt.Errorf("term0Atom(%d): %w", maxPriority, err)
	}
	return a, ok, nil
}

func (p *parser) term0Atom(maxPriority int16) (term.Cell, bool, error) {
	a, ok, err := p.atom()
	if err != nil {
		return term.Cell{}, false, fmt.Errorf("atom(): %w", err)
	}
	if !ok {
		return term.Cell{}, false, nil
	}
	...
	if err != nil {
		return term.Cell{}, false, fmt.Errorf("functionalNotation(%s): %w", a, err)
	}
	if !ok {
		return term.Cell{}, false, nil
	}
...
func (p *parser) variable(s string) (term.Cell, error) {
...
func (p *parser) openClose() (term.Cell, bool, error) {
	t, ok, err := p.term(1201)
	if err != nil {
		return term.Cell{}, false, fmt.Errorf("term(1201): %w", err)
	}
	if !ok {
		return term.Cell{}, false, nil
	}
...
func (p *parser) list() (term.Cell, bool, error) {
	var elems []term.Cell
	arg, err := p.arg()
	if err != nil {
		return term.Cell{}, false, err
...
func (p *parser) curlyBracketedTerm() (term.Cell, bool, error) {
	t, ok, err := p.term(1201)
	if err != nil {
		return term.Cell{}, false, fmt.Errorf("term(1201): %w", err)
	}
...
func (p *parser) functionalNotation(functor term.Atom) (term.Cell, bool, error) {
	switch t, _ := p.next(); t.kind {
	case tokenOpenCT:
		arg, err := p.arg()
		if err != nil {
			return term.Cell{}, false, fmt.Errorf("arg(): %w", err)
		}
		args := []term.Cell{arg}
		for {
			switch t, _ := p.next(); t.kind {
...
func (p *parser) arg() (term.Cell, error) {
	arg, ok, err := p.atom()
	if err != nil {
		return term.Cell{}, fmt.Errorf("atom(): %w", err)
	}
	if ok {
	...
	t, ok, err := p.term(999)
	if err != nil {
		return term.Cell{}, fmt.Errorf("term(999): %w", err)
	}
	if !ok {
		return term.Cell{}, fmt.Errorf("term(999): %w", &UnexpectedTokenError{token: p.current()})
	}
	return t, nil
}

func (p *parser) integer(sign int64, s string) (term.Cell, error) {
	base := 10
	switch {
	...
	case big.Above:
		return term.Cell{}, ErrIntAbove
	case big.Below:
		return term.Cell{}, ErrIntBelow
	default:
		return p.arena.PutInteger(i)
	}
}

func (p *parser) float(sign float64, s string) (term.Cell, error) {
