...
func TestNew_variableNames(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "32", query: `read_term(T,[variable_names(VN_list)]), VN_list=[_=1,_=2,_=3], writeq(VN_list).`, err: context.DeadlineExceeded, ctxErr: context.DeadlineExceeded},
		...
		{name: "45", query: `read_term(T,[variable_names(VN_list)]).`, ctxErr: context.DeadlineExceeded},
		{name: "46", input: `a.`, query: `read_term(T,[variable_names(VN_list)]), T = a, VN_list = [].`},
		{name: "47", query: `VN_list = 42, read_term(T,[variable_names(VN_list)]).`, ctxErr: context.DeadlineExceeded},
		...
		{name: "69", query: `read_term(T, [singletons(1)]).`, ctxErr: context.DeadlineExceeded},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.ctxErr, ctx.Err())
		})
	}
}
