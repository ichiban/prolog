...
func Assertz(vm *VM, t Term, k Cont, env *Env) *Promise {
	m := vm.Module()
	if err := assertMerge(m, t, func(existing, new []clause) []clause {
...
func assertMerge(m *Module, t Term, merge func([]clause, []clause) []clause, env *Env) error {
	...
	case Compound:
		...
		case Atom:
			...
			case Integer:
				...
				if u, ok := m.procedures[key].(*userDefined); !ok || !u.dynamic {
